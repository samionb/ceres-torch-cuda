...
| Trust region minimizer | `trust_region_minimizer.cc` | `ceres_torch.solver` | implemented | LM convergence and minimum-radius termination, traditional/subspace dogleg steps, Ceres-style LM radius updates/rejection thresholds, projected constrained line search, nonmonotonic step evaluator with best-state restoration, Ceres-style inner-iteration disabling/accounting, progress logging, and detailed timing counters |
